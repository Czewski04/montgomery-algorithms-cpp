...
void MontProImpl::runM() {
    uint64 p, rinv, phi, plo, m, a, b, abar, bbar;
    a=8;
    b=13;
    m=19;
    abar = modul64(a,0,m);
    bbar = modul64(b,0,m);
    p = montmul(abar,bbar,m,5);
    mulul64(p, 3, &phi, &plo);
    p = modul64(phi, plo, m);
    std::cout<<"p: " << p << std::endl;
}

void MontProImpl::xbinGCD(uint64 a, uint64 b, uint64 *pu, uint64 *pv)
{
...
uint64 MontProImpl::modul64(uint64 x, uint64 y, uint64 z) {
...
void MontProImpl::mulul64(uint64 u, uint64 v, uint64 *whi, uint64 *wlo)
{
...
uint64 MontProImpl::montmul(uint64 abar, uint64 bbar, uint64 m,
uint64 mprime) {
